...
- 修复图表渲染问题
- 修复移动端布局问题
- 修复server.py中SSL_ENABLED变量未声明global的问题
- 修复项目卡片缺少趋势按钮的问题，现在用户可以直接访问Bug趋势和预测功能
- 修复趋势分析界面"项目未找到"的问题，添加了模拟数据的数据库持久化
- 在趋势分析界面添加了返回按钮，方便用户返回仪表盘
- 修复项目详情界面看不到bug信息的问题，确保与仪表盘显示的各级bug数量信息一致
- 优化了GitLab API不可用时的后备机制，确保系统在离线状态下也能正常工作
